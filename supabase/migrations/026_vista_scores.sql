-- 026: Vista v_scores_empleados
-- Consumida por dashboard_gerencia.jsx para rankings mensuales.
-- Columnas esperadas: empleado_id, empresa_id, nombre, apodo,
--   dias_fichados, horas_trabajadas, tardanzas, solicitudes_mes,
--   score_em (empleado del mes), score_nc (no colaborativo).

CREATE OR REPLACE VIEW v_scores_empleados AS
WITH mes AS (
...
SELECT e.id AS empleado_id,
       ...
       -- score_em: puntaje positivo (más días + más horas - tardanzas)
       ROUND(
       ...
       , 1) AS score_em,
       -- score_nc: puntaje negativo (tardanzas + solicitudes - días trabajados)
       ROUND(
         LEAST(100, GREATEST(0,
