-- 024: Fix timezone bug en auto_fichar_egresos
-- El cast (v_fecha_local || ' ' || v_hora_out)::timestamptz usaba el timezone
-- del servidor (UTC) en vez del timezone de la empresa. Esto causaba que
-- registro_actividades.hora_fin se guardara con offset incorrecto.
  ...
  LOOP
    ...
    IF v_hora_out IS NOT NULL
      ...
      UPDATE registro_actividades
         SET hora_fin = (v_fecha_local || ' ' || v_hora_out)::timestamp AT TIME ZONE v_emp.tz
