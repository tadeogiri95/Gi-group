...
import { fH, fB, fM, setColoresEmpresa } from './lib/theme';
...
export default function OnboardingWizard({ empresa, usuario, onComplete }) {
  ...
  // ─── Estilos ───
  const card = { background: V.surface, borderRadius: 16, padding: 16, border: `1px solid ${V.border}`, marginBottom: 12 };
  const input = { width: "100%", padding: "10px 12px", borderRadius: 10, background: V.surfHi, border: `1px solid ${V.border}`, color: V.text, fontSize: 13, fontFamily: fB, outline: "none", boxSizing: "border-box" };
  ...
  const btnS = { padding: "12px 20px", borderRadius: 12, border: `1px solid ${V.border}`, background: "transparent", color: V.dim, fontSize: 14, fontWeight: 600, fontFamily: fB, cursor: "pointer" };

  return (
    <div style={{ flex: 1, overflowY: "auto", padding: "0 18px 24px", fontFamily: fB }}>
      {/* Header con progreso */}
      <div style={{ padding: "20px 0 16px", borderBottom: `1px solid ${V.border}`, marginBottom: 16 }}>
        <div style={{ fontSize: 11, color: V.amber, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em" }}>Configuración inicial</div>
        <h1 style={{ margin: "4px 0 12px", fontFamily: fH, fontSize: 22, fontWeight: 700, color: V.text }}>Paso {step} de 4</h1>
        <div style={{ display: "flex", gap: 6 }}>
          {[1, 2, 3, 4].map(s => <div key={s} style={{ flex: 1, height: 4, borderRadius: 2, background: s <= step ? V.amber : V.surfHi }} />)}
        </div>
      </div>

      {/* PASO 1: RUBRO + PLANTILLA */}
      {step === 1 && <>
        <h2 style={{ margin: "0 0 6px", fontFamily: fH, fontSize: 18, fontWeight: 700, color: V.text }}>¿De qué rubro es tu empresa?</h2>
        <p style={{ fontSize: 12, color: V.dim, marginBottom: 14 }}>Elegí uno y te proponemos divisiones y etapas. Podés editarlas debajo.</p>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginBottom: 16 }}>
          {Object.entries(PLANTILLAS).map(([k, v]) => (
            <button key={k} onClick={() => aplicarPlantilla(k)} style={{ padding: "10px 8px", borderRadius: 10, border: `1px solid ${rubro === k ? V.amber : V.border}`, background: rubro === k ? `${V.amber}15` : V.surface, color: rubro === k ? V.amber : V.text, fontSize: 12, fontWeight: 600, fontFamily: fB, cursor: "pointer", textAlign: "left", display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <div style={card}>
            ...
            {divisiones.map((d, i) => (
              <div key={i} style={{ display: "grid", gridTemplateColumns: "40px 60px 1fr 32px 32px", gap: 6, marginBottom: 6, alignItems: "center" }}>
                ...
                <input type="color" value={d.color} onChange={e => updDiv(i, "color", e.target.value)} style={{ width: 32, height: 36, border: `1px solid ${V.border}`, borderRadius: 8, cursor: "pointer", background: "transparent" }} />
                <button onClick={() => delDiv(i)} style={{ width: 32, height: 36, borderRadius: 8, border: "none", background: `${V.red}18`, color: V.red, cursor: "pointer", fontSize: 14 }}>✕</button>
              </div>
            ))}
            {divisiones.length === 0 && <div style={{ padding: 14, textAlign: "center", color: V.dim, fontSize: 12 }}>Sin divisiones — agregá una o aplicá una plantilla</div>}
          </div>
          ...
          <div style={card}>
            ...
            {etapas.map((e, i) => (
              <div key={i} style={{ display: "grid", gridTemplateColumns: "40px 50px 1fr 32px 32px", gap: 6, marginBottom: 6, alignItems: "center" }}>
                ...
                <input type="color" value={e.color} onChange={ev => updEt(i, "color", ev.target.value)} style={{ width: 32, height: 36, border: `1px solid ${V.border}`, borderRadius: 8, cursor: "pointer", background: "transparent" }} />
                <button onClick={() => delEt(i)} style={{ width: 32, height: 36, borderRadius: 8, border: "none", background: `${V.red}18`, color: V.red, cursor: "pointer", fontSize: 14 }}>✕</button>
              </div>
            ))}
            {etapas.length === 0 && <div style={{ padding: 14, textAlign: "center", color: V.dim, fontSize: 12 }}>Sin etapas</div>}
          </div>
        </>}
      ...
      {step === 2 && <>
        <h2 style={{ margin: "0 0 6px", fontFamily: fH, fontSize: 18, fontWeight: 700, color: V.text }}>Personalización</h2>
        <p style={{ fontSize: 12, color: V.dim, marginBottom: 14 }}>Logo y colores. Podés saltarlo y editarlo después.</p>

        <div style={card}>
          <label style={lbl}>Logo</label>
          {logoPreview && <div style={{ textAlign: "center", marginBottom: 10 }}>
            <img src={logoPreview} alt="logo" style={{ maxWidth: 120, maxHeight: 120, borderRadius: 12, background: V.bg, padding: 6 }} />
          </div>}
          <input ref={fileLogoRef} type="file" accept="image/*" hidden onChange={onLogoFile} />
          <button onClick={() => fileLogoRef.current?.click()} style={{ width: "100%", padding: 10, borderRadius: 10, border: `1px solid ${V.border}`, background: V.surfHi, color: V.text, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
        ...
        <div style={card}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
            <input type="color" value={colorPrim} onChange={e => setColorPrim(e.target.value)} style={{ width: 50, height: 40, border: `1px solid ${V.border}`, borderRadius: 8, cursor: "pointer", background: "transparent" }} />
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <input type="color" value={colorSec} onChange={e => setColorSec(e.target.value)} style={{ width: 50, height: 40, border: `1px solid ${V.border}`, borderRadius: 8, cursor: "pointer", background: "transparent" }} />
      ...
      {/* PASO 3: EMPLEADOS */}
      {step === 3 && <>
        <h2 style={{ margin: "0 0 6px", fontFamily: fH, fontSize: 18, fontWeight: 700, color: V.text }}>Cargá empleados</h2>
        <p style={{ fontSize: 12, color: V.dim, marginBottom: 14 }}>Manual, CSV, o saltá y hacelo después.</p>

        <div style={card}>
          <label style={lbl}>Importar CSV</label>
          <p style={{ fontSize: 11, color: V.dim, marginBottom: 8 }}>Columnas: <code>nombre, legajo, division, rol</code> (la primera fila debe tener los encabezados)</p>
          <input ref={fileCsvRef} type="file" accept=".csv,text/csv" hidden onChange={onCsvFile} />
          <button onClick={() => fileCsvRef.current?.click()} style={{ width: "100%", padding: 10, borderRadius: 10, border: `1px solid ${V.cyan}40`, background: `${V.cyan}10`, color: V.cyan, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>📤 Subir archivo .csv</button>
        </div>
        ...
        <div style={card}>
          ...
          {empleados.map((e, i) => (
            <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 70px 90px 32px", gap: 6, marginBottom: 6, alignItems: "center" }}>
              ...
              <button onClick={() => delEmp(i)} style={{ width: 32, height: 36, borderRadius: 8, border: "none", background: `${V.red}18`, color: V.red, cursor: "pointer", fontSize: 14 }}>✕</button>
            </div>
          ))}
          {empleados.length === 0 && <div style={{ padding: 14, textAlign: "center", color: V.dim, fontSize: 12 }}>Sin empleados todavía</div>}
        </div>
      ...
      {/* PASO 4: RESUMEN */}
      {step === 4 && <>
        <h2 style={{ margin: "0 0 6px", fontFamily: fH, fontSize: 18, fontWeight: 700, color: V.text }}>Listo para empezar</h2>
        <p style={{ fontSize: 12, color: V.dim, marginBottom: 16 }}>Revisá la configuración y confirmá.</p>

        <div style={card}>
          <div style={{ display: "flex", justifyContent: "space-between", padding: "6px 0" }}><span style={{ color: V.dim, fontSize: 13 }}>Rubro</span><span style={{ color: V.text, fontWeight: 600, fontSize: 13 }}>{rubro ? PLANTILLAS[rubro]?.label : "Sin definir"}</span></div>
          <div style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderTop: `1px solid ${V.border}` }}><span style={{ color: V.dim, fontSize: 13 }}>Divisiones</span><span style={{ color: V.text, fontWeight: 600, fontSize: 13 }}>{divisiones.length}</span></div>
          <div style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderTop: `1px solid ${V.border}` }}><span style={{ color: V.dim, fontSize: 13 }}>Etapas</span><span style={{ color: V.text, fontWeight: 600, fontSize: 13 }}>{etapas.length}</span></div>
          <div style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderTop: `1px solid ${V.border}` }}><span style={{ color: V.dim, fontSize: 13 }}>Logo</span><span style={{ color: V.text, fontWeight: 600, fontSize: 13 }}>{logoPreview ? "✅ Cargado" : "—"}</span></div>
          <div style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderTop: `1px solid ${V.border}` }}><span style={{ color: V.dim, fontSize: 13 }}>Colores</span><span style={{ display: "flex", gap: 4 }}><span style={{ width: 18, height: 18, borderRadius: 4, background: colorPrim }} /><span style={{ width: 18, height: 18, borderRadius: 4, background: colorSec }} /></span></div>
          <div style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderTop: `1px solid ${V.border}` }}><span style={{ color: V.dim, fontSize: 13 }}>Empleados</span><span style={{ color: V.text, fontWeight: 600, fontSize: 13 }}>{empleados.length}</span></div>
        </div>

        {error && <div style={{ padding: 12, background: `${V.red}15`, color: V.red, borderRadius: 10, fontSize: 12, marginBottom: 10 }}>{error}</div>}
