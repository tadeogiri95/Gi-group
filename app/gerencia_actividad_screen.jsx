...
export default function GerenciaActividadScreen({ empresaId }) {
  ...
  const getTipoActividad = (etapaCodigo) => {
    if (etapaCodigo === 0) return { label: "Espera", color: C.red, bg: C.redS };
    if (etapaCodigo > 0) return { label: "Productivo", color: C.green, bg: C.greenS };
    return { label: "Improductivo", color: C.mute, bg: C.surfLo };
  };
                      ...
                      .map(op => {
                        ...
                        return (
                          <div key={op.empleado_id} className="bg-gypi-surface rounded-[14px] overflow-hidden" style={{
                            ...
                            {isExpanded && (
                              <div style={{ borderTop: `1px solid ${C.border}`, background: C.surfLo, padding: "12px 14px" }}>
                                {/* Fichada del día */}
                                {fichadaDetalle && (
                                  <div className="flex flex-wrap gap-2.5 mb-3 text-[11px]">
                                    ...
                                    {parseFloat(fichadaDetalle.horas_extra) > 0 && (
                                      <div className="flex items-center gap-1 px-1.5 py-0.5 rounded" style={{ background: `${C.amber}20` }}>
                                        <span className="font-bold" style={{ color: C.amber }}>+{parseFloat(fichadaDetalle.horas_extra).toFixed(1)}h extra</span>
                                      </div>
                                    )}
                                    {fichadaDetalle.llegada_tarde && (
                                      <div className="flex items-center gap-1 px-1.5 py-0.5 rounded" style={{ background: C.redS }}>
                                        <span className="font-bold" style={{ color: C.red }}>Tarde {fichadaDetalle.minutos_tarde}min</span>
                                      </div>
                                    )}
                                  </div>
                                )}
                                ...
                                {loadingDetalle ? (
                                  ...
                                  <div className="flex flex-col gap-1.5">
                                    {actividades.map((act, idx) => {
                                      ...
                                      return (
                                        <div key={act.id || idx} className="rounded-[10px] p-2.5" style={{
                                          ...
                                          border: `1px solid ${enCurso ? `${tipoAct.color}40` : C.border}`,
                                        }}>
                                          ...
                                          <div className="flex items-center gap-2 mb-1">
                                            ...
                                            {enCurso && (
                                              <span className="text-[10px] font-bold px-1.5 py-0.5 rounded" style={{ background: `${C.amber}20`, color: C.amber }}>
                                                EN CURSO
                                              </span>
                                            )}
                                          </div>
                                          ...
                                          <div className="flex items-center gap-1.5 text-[11px]">
                                            {act.codigo_proyecto && (
                                              <span className="font-mono font-semibold" style={{ color: C.amber }}>
                                            ...
                                            {act.etapa === 0 && (
                                              <span style={{ color: C.red }}>⏸ En espera</span>
                                            )}
                                    ...
                                    {/* Resumen del detalle */}
                                    <div className="mt-1 pt-2 flex gap-3 text-[10px] text-gypi-dim" style={{ borderTop: `1px solid ${C.border}` }}>
