...
//
// ENTREGA 2F: Separado de page.js porque useSearchParams
// requiere estar dentro de un Suspense boundary.
// ═══════════════════════════════════════════════════════════

import { useState, useEffect } from "react";
import { useParams, useSearchParams, useRouter, usePathname } from "next/navigation";
import { C, fH, fB, setColoresEmpresa } from "../lib/theme";
...
export default function HomeContent() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [empleados, fichadasHoy, miFichada, fichadasSemana, solicitudes, misSolicitudes, reglas, notificaciones] = await Promise.all([
        ...
        sb.get(isG ? "notificaciones?destinatario_rol=eq.gerencial&order=created_at.desc&limit=10" : `notificaciones?destinatario_rol=eq.${usuario.legajo}&order=created_at.desc&limit=10`),
      ]);
  ...
  const logout = () => { setUsuario(null); clearToken(); try { localStorage.removeItem("gi-session"); localStorage.removeItem("gi-session-time"); } catch {} router.push("/"); };
  useEffect(() => { onUnauthorized(() => { setUsuario(null); clearToken(); try { localStorage.removeItem("gi-session"); localStorage.removeItem("gi-session-time"); } catch {} }); }, []);
