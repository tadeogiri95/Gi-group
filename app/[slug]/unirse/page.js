...
export default function UnirseScreen() {
  ...
  if (empresaNotFound) {
    return (
      <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100dvh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 28, color: C.text, fontFamily: fB, textAlign: "center" }}>
  ...
  const inputStyle = { width: "100%", padding: "14px 16px", borderRadius: 12, background: C.surface, border: `1px solid ${C.border}`, color: C.text, fontSize: 15, fontFamily: fB, outline: "none", boxSizing: "border-box" };
  const lblStyle = { display: "block", fontSize: 11, fontWeight: 700, color: C.dim, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 8 };
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100dvh", display: "flex", flexDirection: "column", padding: "0 28px", justifyContent: "center", color: C.text, fontFamily: fB }}>
      ...
      {step === 1 && (
        <>
          <h1 style={{ margin: 0, fontFamily: fH, fontSize: 28, fontWeight: 700, color: C.text, letterSpacing: "-0.025em" }}>Unite a {empresa.nombre_corto || empresa.nombre}</h1>
          <p style={{ fontSize: 13, color: C.dim, marginTop: 8, marginBottom: 28, lineHeight: 1.5 }}>
          ...
          {error && <div style={{ padding: 12, background: `${C.red}15`, color: C.red, borderRadius: 10, fontSize: 12, marginTop: 12 }}>{error}</div>}

          <div style={{ textAlign: "center", marginTop: 24, fontSize: 13, color: C.dim }}>
            ¿Ya tenés cuenta?{" "}
            <button onClick={() => router.push(`/${slug}`)} style={{ background: "none", border: "none", color: C.amber, cursor: "pointer", fontSize: 13, fontWeight: 700 }}>
              Iniciar sesión
            </button>
          </div>
        </>
      ...
      {step === 2 && (
        <>
          <div style={{ width: 56, height: 56, borderRadius: 16, background: `${C.green}22`, display: "flex", alignItems: "center", justifyContent: "center", color: C.green, marginBottom: 16, fontSize: 28 }}>✓</div>
          <h1 style={{ margin: 0, fontFamily: fH, fontSize: 24, fontWeight: 700, color: C.text }}>¡Hola, {empleado?.apodo || empleado?.nombre}!</h1>
          <p style={{ fontSize: 13, color: C.dim, marginTop: 8, marginBottom: 24, lineHeight: 1.5 }}>
            Creá tu contraseña para terminar de activar tu cuenta en <b style={{ color: C.text }}>{empleado?.empresaNombre}</b>.
          </p>
          ...
          <button onClick={() => setShowPwd(!showPwd)} style={{ background: "none", border: "none", color: C.dim, cursor: "pointer", fontSize: 12, fontFamily: fB, padding: "4px 0", marginBottom: 16, textAlign: "left" }}>
            {showPwd ? "🙈 Ocultar contraseñas" : "👁️ Mostrar contraseñas"}
          </button>

          <button onClick={activar} disabled={loading || !password || !confirm} style={{ width: "100%", padding: 14, borderRadius: 12, background: password && confirm && !loading ? C.green : C.surface, color: password && confirm && !loading ? "#000" : C.mute, border: "none", fontSize: 15, fontWeight: 700, cursor: password && confirm && !loading ? "pointer" : "default" }}>
      ...
      {step === 3 && (
        <div style={{ textAlign: "center" }}>
          ...
          <h1 style={{ margin: 0, fontFamily: fH, fontSize: 26, fontWeight: 700, color: C.green }}>¡Cuenta activada!</h1>
          <p style={{ fontSize: 14, color: C.dim, marginTop: 12 }}>Redirigiendo al login...</p>
        </div>
      )}
    </div>
  );
