// ═══════════════════════════════════════════════════════════
// Push Notifications — ETAPA 4: filtrado por empresa_id
//
// Cambios respecto a la versión anterior:
// - saveTokenViaAPI guarda empresa_id junto al token push
// - sendPushToLegajo manda empresa_id para filtrar
// - sendPushToRole manda empresa_id para filtrar
// ═══════════════════════════════════════════════════════════
...
export async function requestPushPermission(legajo, empresaId) {
  try {
    ...
    // Guardar token via API route segura con empresa_id
...
// ─── Guardar token via /api/data (con empresa_id) ──
    ...
    if (existing.length > 0) {
      await fetch("/api/data", {
        ...
        body: JSON.stringify({
          method: "PATCH",
          path: `push_tokens?legajo=eq.${legajo}&token=eq.${encodeURIComponent(token)}`,
          body: { updated_at: new Date().toISOString(), empresa_id: empresaId || null },
        }),
      });
    } else {
      await fetch("/api/data", {
        ...
        body: JSON.stringify({
          method: "POST",
          path: "push_tokens",
          body: { legajo, token, empresa_id: empresaId || null },
        }),
      });
    }
    console.log("[Push] Token guardado para legajo:", legajo, "empresa:", empresaId);
  } catch (err) {
...
// ─── Enviar push a un legajo (con empresa_id) ─────
